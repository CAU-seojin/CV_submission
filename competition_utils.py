...
def train_model(ex_dict):
    ...
    ex_dict['Train Results'] = ex_dict['Model'].train(
        ...
        # --- Data Augmentation -----------------------------------
        augment     = ex_dict['augment'],       # 전체 augmentation on/off
        
        mosaic      = ex_dict['mosaic'],        # Mosaic 합성 확률 (0.0~1.0)
        mixup       = ex_dict['mixup'],        # MixUp 합성 확률 (0.0~1.0)
        copy_paste  = ex_dict['copy_paste'],    # Copy-Paste 합성 확률 (0.0~1.0)
        
        hsv_h       = ex_dict['hsv_h'],         # HSV hue 변화 범위 (예: 0.015)
        hsv_s       = ex_dict['hsv_s'],         # HSV saturation 변화 범위 (예: 0.7)
        hsv_v       = ex_dict['hsv_v'],         # HSV value 변화 범위 (예: 0.4)
        
        degrees     = ex_dict['degrees'],       # 회전 각도 범위 (예: 0.0)
        translate   = ex_dict['translate'],     # 이동 비율 (예: 0.1)
        scale       = ex_dict['scale'],         # 크기 조정 비율 (예: 0.5)
        
        flipud      = ex_dict['flipud'],        # 수직 뒤집기 확률 (예: 0.0)
        fliplr      = ex_dict['fliplr'],        # 수평 뒤집기 확률 (예: 0.5)
        # ----------------------------------------------------------

        # (Loss weight tuning — 보통 수정하지 않아도 됩니다)
        # box         = ex_dict['box']           # 박스 손실 가중치
        # cls         = ex_dict['cls']           # 분류 손실 가중치
        # dfl         = ex_dict['dfl']           # DFL 손실 가중치
    )
